...
| L4 worker 여분 | 편성이 cso 다음 worker 를 `javis_boot_node` 로 띄울 때, 자식 폴더 상속 `_master_seat_cwd` 가 **첫 master 좌석 = s9(홈)** 의 폴더를 물려준다 → 복원의 worker(s12 · +34.1s)보다 3초 늦게 `launch-agent --role worker --cwd /Users/admin`(s13 · +37.3s) → 데몬 dedup 이 `worker-2` 로 등록. topology 에 영속돼 **재부팅마다 +1**(2회차 = worker-2 2자리) | `javis_formation.py:481-499`(첫 master 의 cwd) · `:1117-1121` · `javis_boot_node.py:1480-1535`(row 없으면 launch) · `state.rs:3828-3839`(`dedup_worker_role`) · 【관측】 `surface.created s13 {cwd:/Users/admin, role:worker, caller_pid:8062}` → 목록 `worker-2` · 재부팅 2회차 s18·s19 worker-2 |
...
- 【관측】 1.1.2(c112 · 재부팅 1회차)도 **같은 자리표**가 있다: dept-3 `daemon.started 17:01:54` 와 같은 초에 `surface.created s4 {cwd:/Users/admin, role:master}` — 즉 L1·L2 는 **1.1.2 에도 있던 결함**이다.
