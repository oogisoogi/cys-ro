...
#   hard-block 이면 RESOURCE_GATE_RECHECK_S 간격으로 최대 RESOURCE_GATE_RECHECKS 회 다시 잰다 —
...
def _measure_resource_gate(py, gate):
    """게이트 1회 측정 → (code, verdict, why, out). 부작용 = 게이트 subprocess(+exit 2 면 cys list)뿐."""
...
def _run_resource_gate(py, log):
    ...
    if not os.path.isfile(gate):
        ...
        return None
    code, verdict, why, out = _measure_resource_gate(py, gate)
    log.step(STEP.RESOURCE_GATE, code, "결손>0 · verdict=%s · %s\n%s" % (verdict, why, out))
    recheck = 0
    while verdict == "hard-block" and recheck < RESOURCE_GATE_RECHECKS:
        recheck += 1
