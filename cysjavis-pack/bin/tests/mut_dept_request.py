...
M = [
 ...
 ("M20-claimed-dept-name", "codex F3 — --all 이 옛 dept_name 으로 번호를 가림",
  ...
  "        claimed = {reg_name_for_key(r.get(\"key\"), reg) for r in reqs if r.get(\"key\")} | {r.get(\"dept_name\") for r in reqs}\n",
 ...
 ("M23-marker-only", "codex F6 — 표식만 보고 덮기(사람이 고친 생성 파일)",
  "        if not claude_md_marker(dest) or not _generated_untouched(dest):\n",
  "        if not claude_md_marker(dest):\n",
