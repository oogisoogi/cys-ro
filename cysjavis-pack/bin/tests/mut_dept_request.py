...
M = [
 ...
 ("M4-gap-off", "연쇄 — 재호출 물러서기 10분 끄기(v113: 간격은 재호출에만)",
  "    if reentry and t - last < CREATE_GAP_SEC():\n        r[\"waiting_gap\"] = True\n",
  "    if False:\n        r[\"waiting_gap\"] = True\n",
  ["test_f5_retry_respects_gap"]),
 ("M5-fixed-cap-back", "v113 — 틱에 고정 상한 2 되살리기(오너 결정 「고정 상한 제거」 역행)",
  "    last = (st.get(\"last_create_at\") or 0)\n    if reentry and t - last",
  "    if len(live_depts(reg, ts)) >= 2:\n        _fail(r, \"cap:2\")\n        return\n    last = (st.get(\"last_create_at\") or 0)\n    if reentry and t - last",
  ["test_no_fixed_cap_at_tick"]),
 ...
 ("M10-lane-off", "4군① — 레인 판정 끄기",
  ...
  "    return True\n    env = os.environ if env is None else env\n    s = env.get(\"CYS_SOCKET\", \"\") or \"\"\n",
  ["test_propose_no_fixed_cap_resource_only_and_lane", "test_self_test_passes"]),
 ...
 ("M21-busy-no-alternative", "v113 — 자원 거부 문장에서 대안(부서 하나 닫기) 빼기",
  "    if n_live:\n        s += ",
  "    if False:\n        s += ",
  ["test_propose_no_fixed_cap_resource_only_and_lane"]),
 ("M21b-propose-fixed-cap-back", "v113 — 제안 시점 고정 상한 2 되살리기",
  "    gate = resource_check()\n    if gate.get(\"verdict\") == \"hard_block\":\n        return _refuse(busy_say(len(live)), reason=\"resource\")\n",
  "    if len(live) >= 2:\n        return _refuse(\"상한\", reason=\"cap\")\n    gate = resource_check()\n    if gate.get(\"verdict\") == \"hard_block\":\n        return _refuse(busy_say(len(live)), reason=\"resource\")\n",
  ["test_propose_no_fixed_cap_resource_only_and_lane"]),
 ("M22-gap-on-first-create", "v113 — 첫 생성에도 간격 10분(재호출 한정 해제)",
  "    if reentry and t - last < CREATE_GAP_SEC():\n        r[\"waiting_gap\"] = True\n",
  "    if t - last < CREATE_GAP_SEC():\n        r[\"waiting_gap\"] = True\n",
  ["test_one_create_per_tick_and_no_first_gap"]),
 ...
 ("M46-human-axis-off", "v113 A1 — confirm 의 사람 확인 축 끄기(LLM 자기 확인 개방)",
  "    if r.get(\"human_axis\") and not human_ack_after(r):\n",
  "    if False and not human_ack_after(r):\n",
  ["test_human_axis_required_when_hook_active"]),
 ("M47-notice-is-human", "v113 A1 — 기계 알림([부서결과])을 사람 확인으로 기록",
  "    if _NOTICE.match(prompt or \"\"):\n        return True, \"notice\"\n",
  "    if False:\n        return True, \"notice\"\n",
  ["test_human_axis_required_when_hook_active"]),
 ("M48-guide-every-turn", "v113 A1 — 요지 세션 반복 억제 끄기(4군① 폭주 주입)",
  "        if notice or (intent and not seen.get(\"guide\")):\n",
  "        if notice or intent:\n",
  ["test_guide_once_per_session_and_short"]),
 ("M49-news-repeat", "v113 A1 — 부서 소식 세션 중복 억제 끄기",
  "            if k in seen[\"news\"]:\n                continue\n",
  "            if False:\n                continue\n",
  ["test_notice_always_guides_and_news_once"]),
]
