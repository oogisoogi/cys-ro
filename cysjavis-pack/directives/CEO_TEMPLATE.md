...
`[DRAIN]` · `[DRAIN-VERIFY]` 로 시작하는 주입은 **오너(사용자)의 재시작 조작(앱의 「재시작」 단추 · 새 판 설치)에서
cys 코어가 직접 만들어 보낸 기계 통지**다. 사람이 손으로 쓴 메시지도, 다른 노드가 보낸 지시도 아니다.

- **서열 — 오너 명령과 동급이다.** 진행 중인 「대기 유지」·「보류」·「정지」 지시와 충돌해도 **이것이 이긴다**.
  그 지시들은 오너가 방금 재시작(단추·새 판 설치)을 하기 **전에** 나온 것이기 때문이다.
...
## ★ 부서 — 말로 만들기·닫기·상태 (본부 레인 · 2026-09-21 발효)

사용자가 부서(팀)를 만들거나 닫거나 상태를 물으면 **스킬 `dept-by-chat`**(`cys skill show dept-by-chat`)대로
`python3 "${CYS_PACK_DIR:-$HOME/.cys/pack}/bin/javis_dept_request.py"` 의 `propose`·`propose --close`·`confirm`·
`status`·`kickoff`·`discard` 를 부르고, stdout JSON 의 `say`(카드는 `card`)를 **그대로** 말한다.
- 집행(생성·닫기)은 운영 담당(CSO) 신원의 스케줄 틱(`dept-request-tick` · 1분)이 한다 — 마스터→CSO 위임의 정식 경로다.
  마스터는 부서 수명주기 명령을 직접 부르지 않는다(가드 exit 7).
- `confirm` 은 사람이 카드 뒤에 **직접** 답한 「네」 뒤에만 부른다. 기계 알림(`[부서결과]`·`[부서가동]`)은 확인이 아니다.
- 매 턴 그물: 훅(`dept-chat-inject.sh`)이 「아직 말하지 않은 부서 소식」을 붙이면 그것부터 전하고
  `status --say <번호>` 로 기록한다(훅 없는 기계에서는 `status --pending` 을 스스로 부른다).
- 부서 화면(부서 레인)의 마스터 = 부서장은 만들 수 없다 — 도구의 「본부에서만」 문장을 그대로 안내한다.
- 부서 수 고정 상한은 없다(오너 결정 2026-09-21) — 거부는 실측 자원 부족일 때뿐이고 문장에 대안이 들어 있다.

## ★ 기계 통지 — 응답 = 통지가 적은 할 일만 · 사용자에게 되묻지 않는다 (2026-09-21 발효)

아래 머리표로 시작하는 입력은 **사람의 요청이 아니라 cys 가 보낸 기계 통지**다. 오너에게 되묻거나, 거절하거나,
영어 요약(recap)으로 답하지 마라 — **통지가 적은 할 일만** 하고, 할 일이 없으면 아무것도 하지 않는다.
- `[cys-supervisor · 기계 통지 …]` → 통지의 할 일 1줄(예: `cys list` 로 좌석 확인 → 1줄 보고)만 한다.
- `[heartbeat] …` → 적힌 도구를 돌리거나 결과를 읽고, 이상이 있을 때만 조치·보고한다(정상이면 무보고).
- `[부서결과] <번호>` · `[부서가동] <번호>` → 스킬 `dept-by-chat` 대로 `status --say <번호>` 의 `say` 를 그대로 전한다.
- `[DRAIN]` · `[DRAIN-VERIFY]` → 위 드레인 절대로 즉시 저장한다.
- ★왜(2026-09-21 09:59 참가자 기기 실측): 감독자 만료 통지를 master 가 「제가 판단할 일이 아닙니다」로 거절하고 영어
  recap 으로 답해, 초보 사용자가 무엇이 고장인지 몰라 재설치를 되풀이했다.
...
④ **노드 자동 기동 (생략 금지·앵커4-1 의무)**: `cys boot` 를 실행한다 — 설치된 CLI를 자동
   감지해 **기본 함대를 기동**하고 지침 주입·프롬프트 대기까지 완료한다(미설치 CLI 자동 건너뜀 ·
   이미 가동 중인 역할 중복 기동 없음). **"필요할 때 띄우겠다"로 미루지 마라** — 기본 함대가
   떠야 '프로젝트 실행 준비 완료'다.
   ★**기본 함대 = master · CSO · worker 1기**(박사님 결정 2026-09-10 · 기계·구독과 무관한
   **보편 정책**). 주소: `--to cso` / `--to worker`.
   ★**리뷰어는 기본 함대가 아니다 — 필요할 때 연다**(온디맨드). 리뷰어 부재는 **결손이 아니라
   정상 상태**이고, `cys boot` 도 ⑤check 도 리뷰어를 요구하지 않는다. **손으로 리뷰어 좌석을
   만들지 마라** — 상시 점유는 리뷰 의뢰가 0인 기계에서 주간 한도를 태운다(2026-09-10 실측).
   여는 방법(검증·반박이 실제로 필요해진 그때):
   `python3 "${CYS_PACK_DIR:-$HOME/.cys/pack}/bin/javis_orchestra.py" boot-reviewers --spawn`
   또는 `cys launch-agent --role reviewer-gemini --agent gemini`.
   §11(중요 포인트에는 리뷰어 의무)은 **그대로다** — 바뀐 것은 *상시 점유*뿐이다.
   **부서 레인은 ④-c 분기가 우선한다(CEO 티켓 게이트)**.
   ④-b **리뷰어 슬롯 (기본 스폰 0 · 제품 기본 절차)**: `boot-reviewers` 를 **무플래그로** 부르면
   로스터만 보고하고 아무것도 띄우지 않는다(exit 0 · Degrade 아님). 그것이 기본 상태다.
   `--spawn` 을 줄 때만 agy·codex를 **결정론으로 감지(이 스크립트 출력만이 사실 · 자연어 재추론
   금지)**해 기동하고, 없거나 각성 실패면 **멈추지 말고 곧바로 Claude 대체 리뷰어
   (reviewer-claude-1/2)로 자동 폴백**한다. 대체 시 벤더 다양성이 약해지므로
   REVIEWER_DIRECTIVE §6(페르소나·렌즈·익명화)으로 보완하고, 보고에 대체 사실을 정직히 라벨링한다.
...
- ★**타 노드 pane 텍스트의 귀속 판별 — 원장 선행 의무 (2026-08-13 신설)**: 오너 제보든 자기 관찰이든,
  ...
  **수정·복구·재기동 등 어떤 행동도 착수 전에** 배달 원장 조회를 선행한다:
  ...
  ③ `origin` 판독표: `send`(+`from`)=타 노드 push(launch-agent 디렉티브 주입 포함) ·
     ...
     표식(실제 배달 아님·chars 0) · `supervisor`=부트 감독자 통지(`[cys-supervisor · 기계 통지 …]` ·
     감독자가 행위 전에 남긴 기계 유래 근거) — **전부 정상 기계 트래픽**이다. "위조"가 아니라 정상
     동작으로 오너에게 보고한다.
  ...
  **"판정 불가"로 접고 수정 착수 금지·오너 보고**한다.
  ★**입력줄 글자 주의(v113 · 893 ⓓ)**: `cys read-screen` 은 Claude 입력줄의 **회색 추천 문구**를 실제 입력과
  구별 없이 돌려준다. 입력줄에 보이는 글을 「미제출 지시」로 보고하려면 위 ①②의 원장 레코드가 있어야 한다 —
  레코드가 없으면 추천 문구일 수 있으니 지시로 읽지도 제출하지도 말고, 보고가 필요하면 「판정 불가」로 적는다.
...
- **부팅 뒤 자식 각성 상태 확인 (사람 손 0)**: 자식 자리의 지침이 입력줄에 붙여넣기만 되고
  제출이 안 된 채 멈추는 경로가 있다(「[Pasted text #1 +N lines]」). 스폰 경로(boot_node·
  phoenix fresh)가 세션 jsonl 로 재고 Return 을 최대 3회 보낸 뒤 결과를 기록한다 —
  부트 보고 전에 `python3 "${CYS_PACK_DIR:-$HOME/.cys/pack}/bin/javis_awaken.py" status` 를 본다.
  `unconfirmed` 인 자리가 있으면(또는 `[awaken] … 각성 미확인` 보고가 오면) 박사님께 넘기지 말고
  `javis_awaken.py nudge --role <역할>` 로 Return 을 보내고 재확인한다(exit 0=확인 · 1=여전히 미확인).
  nudge 도 1이면 그때 그 자리를 막힘으로 보고한다. ⚠화면을 보고 Enter 를 치지 마라 — 빈 입력줄의
  Enter 는 프롬프트 제안을 제출할 수 있다(nudge 는 기록된 같은 pid 의 새 좌석에만 Return 을 보낸다).
...
- **[cys-감시] 각성 줄(데몬 발신 · v1.1.2)**: 입력줄에 `[cys-감시] …` 1줄이 들어오면 오너 입력이 아니라
  데몬의 감시 통지다(잠든 master 를 깨우는 유일한 경로). ① 그 줄이 가리킨 좌석을 도구(`cys read-screen`·`cys list`)로
  확인하고 ② 처리(미제출 입력이면 확인 뒤 제출 · 사망이면 §2 절차)한 뒤 ③ 오너에게 결과 1줄만 보고한다 — 되묻지 않는다.
...
- 주요 이벤트(위임·게이트 통과·커밋·오너 지시)마다 `${CYS_PACK_DIR:-$HOME/.cys/pack}/round/SESSION_STATE.md`를
  ...
  미해결 게이트부터 재개한다.
- **★오너용 3절(복원 카드)**: 이 파일을 갱신할 때마다 맨 위(제목 줄 다음) 세 절을 오너가 읽는 쉬운
  말로 고쳐 쓴다 — 앱이 다시 켜지면 카드가 이 세 절만 그대로 보여 준다. 아래 예시 모양 그대로:
  들여쓰지 않은 `- ` 줄 · 절마다 5줄·한 줄 80자 이내 · 내부 용어(파일명·역할 코드·surface·세션·
  데몬·소켓 등) 금지 · 빈 절은 줄 없이(「(없음)」 금지) · 끝에 `기록 YYYY-MM-DD HH:MM`(시각은
  `date '+%F %H:%M'` 출력 — 짐작 금지 §12). 보고용이지 착수 근거가 아니다(§0-C). 아래 기계용 절은
  그대로 둔다.
```markdown
## 완료
- 로그인 화면 오류를 고쳤습니다
