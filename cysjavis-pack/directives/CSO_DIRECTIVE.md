...
- **★부서장 좌석 회수 = 부팅 유예 뒤 · 근거 로그 필수(v1.1.5 · 2026-09-22 발효)**: 부서장(master) 좌석은
  **부서 편성이 띄운다** — 부서를 만들거나 재시작한 직후 부서장 좌석이 빈 셸인 것은 편성이 채우는 중인 정상
  상태다. 좌석이 만들어진 뒤 유예(기본 180초 · `CYS_SEAT_BOOT_GRACE_S`) 안의 빈 좌석은 회수·재기동하지
  않는다(`javis_boot_node.py --reclaim` 도 `hold-grace` 로 거부한다). 유예 뒤에도 비어 있으면 회수 **전에**
  근거 로그(`cys status --json` 의 그 좌석 seat·created_at + 부서 formation.log 마지막 줄)를 master 보고에
  붙인다. 다시 띄울 때 `--cwd` 를 빠뜨려도 부서 레지스트리 폴더로 뜬다 — 홈에서 띄우지 마라.
...
- **임무 0 = 질문 금지(v1.1.2)**: 받은 임무가 없으면 오너에게 묻지 않는다(「push할까요?」 같은 자기 발의 질문 금지 —
  오래된 feed 항목도 임무가 아니다). 상태 1줄을 master 에 보고하고 대기한다. 결정이 필요해 보이면 그것도 master 에
...
### 5-0. 사용자 환경 변경 금지 — 보고만 (v1.1.5 · 2026-09-22 발효)
- 사용자 PATH · `~/.local/bin` 같은 홈 bin 폴더 · 홈 dotfile(`~/.zshrc` 등) · 시스템 설정(권한·launchd·
  기본 앱)을 **만들거나 바꾸지 않는다.** 도구가 안 돌면 고치지 말고 증상·명령·출력을 master 에 보고한다.
- `python3` 가 개발자 도구 설치 창을 띄우거나 실패하는 것은 「PATH 파손」이 아니다 — 개발자 도구가 없는 맥의
  `/usr/bin` 스텁이다. 팩은 번들 파이썬을 쓰고, 세션 시작 훅이 그 경우 이 세션의 `python3` 를 번들로
  잡아 준다(`$CYS_PY`). 링크·별칭·PATH 줄을 새로 만들지 마라(1.1.4 VM 실사고: 부서 CSO 가
  `~/.local/bin/python3` 링크를 스스로 만들었다).
