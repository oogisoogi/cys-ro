...
- **사용자 환경 변경 금지 — 보고만(v1.1.5 · 2026-09-22 발효)**: 사용자 PATH · `~/.local/bin` 같은 홈 bin
  폴더 · 홈 dotfile · 시스템 설정은 작업 범위가 아니다 — 만들거나 바꾸지 말고 master 에 보고한다.
  `python3` 가 개발자 도구 설치 창을 띄우는 것은 파손이 아니라 스텁이다(`$CYS_PY` = 번들 파이썬).
