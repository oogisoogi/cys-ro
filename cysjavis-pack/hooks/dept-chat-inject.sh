...
if [ ! -e "$_RQ/.hook-wake" ]; then
  case "$HOOK_IN" in
    # ★dbg-D3 F5: 부서 이름 꼴(…부·…과 + 생성·닫기 동사)도 파이썬 판정으로 넘긴다 — 정밀 판정(일반어 제외)은
    #   javis_dept_request.dept_intent 가 한다. 여기는 싼 상위집합 거름이다(외부 프로세스 0 유지).
    *부서*|*팀*|*'[부서결과]'*|*'[부서가동]'*) ;;
    *부*만들*|*부*꾸려*|*부*꾸리*|*부*생성*|*부*닫*|*부*없애*|*부*해체*|*부*지워*) ;;
    *과*만들*|*과*꾸려*|*과*꾸리*|*과*생성*|*과*닫*|*과*없애*|*과*해체*|*과*지워*) ;;
    *) exit 0 ;;
  esac
fi
