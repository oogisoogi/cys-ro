...
**master 판정 22:4x(`master#26153cb0` · 2026-09-18)** — 코드 변경 0:
- ①알림 계약 = **현행 유지**(최대 1회 + `status --pending` 이 현재 상태를 받침). 중간 전이 보고는 오너 요구가 확인된 뒤. outbox·재시도는 폭주 축을 하나 더 여는 비용.
- ②묘비 해소 재시도 = **현행 유지 + ISSUES B11 세대 ID 로 닫기**(워커 권고 채택).
