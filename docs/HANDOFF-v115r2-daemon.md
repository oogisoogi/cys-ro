...
- ★**TTL 기준점 = 항목의 `enqueued_at`(그 지시가 미배달로 지낸 시간)** — agy r2 개정.
  배치에는 만기 시각이 **없다**(`ParkedQueue.parked_at`·`from_surface` 는 관측·감사 전용이며 **처음** 값을
  보존한다). ⚠내가 r1 봉합에서 적었던 「기준점 = 처음 주차 시각 · 늙은 배치에 병합된 새 항목은 TTL 을
  온전히 못 받는다」는 **폐기된 서술**이다 — r2 가 그 부수 효과를 #2·#3(BLOCK)으로 되돌려줬다.
  `QueueEntry` 스키마 변경이 필요하다고 적었던 것도 **틀렸다**: `enqueued_at` 이 이미 있고 WAL 로
  영속되며 레거시 항목도 복원 시각으로 합성된다(0.0 금지 규약).
...
## 3-3. agy r2 = **BLOCK** → 계열 전환으로 봉합

정본 verdict = `…/D7-2_r2___r1_BLOCK_2건_봉합의_충분성-r1-reviewer1.json`(`{"verdict":"BLOCK",
"evidence":"state.rs:364"}`).

| # | 지적 | 판단 | 처리 |
|---|---|---|---|
| 1 | 무음 유실은 닫혔다 · 다만 아무도 주차·상속하지 않는 역할의 만기분은 정지 상태로 남는다 | 리뷰어는 「한계로 수용 가능」이라 했으나 **수용하지 않았다** | **워치독 15초 틱에 스윕 신설**(`governance::sweep_parked_queues` · 걷이 규칙은 `state::drain_expired_parked_locked` 공유 = 사본 0) |
| 2 | 늙은 배치에 병합된 새 지시가 즉시 만기 | 실재 | ↓ 계열 전환으로 **성립 불가**가 됨 |
| 3 | 상한이 옛 배치를 전량 evict 했는데 살아남은 새 항목이 버려진 배치의 시각을 물려받아 폐기(**BLOCK 사유**) | 실재 | ↓ 같은 전환 |
| 4 | 회귀 없음(호출부 단일·락 밖 발행) | 수용 | 무변경 |
| 5 | `match map.remove(role)` 의 만기 분기가 **죽은 코드** · 병합 때 `from_surface` 덮어쓰기로 **출처 왜곡** | 둘 다 실재 | 죽은 분기 제거 · `from_surface`·`parked_at` 은 **처음** 값 보존 |

★**같은 자리가 두 라운드 연속 뚫렸다**(r1 ⑵ · r2 #2 · r2 #3). 셋 다 **배치 단위 시각 장부**(`base_at`)를
유지하려다 생긴 것이다. ⇒ 술어를 더 정교하게 만드는 대신 **재는 계열을 바꿨다**: 만기는 **항목별
`enqueued_at`** 으로만 재고 배치에는 시각을 두지 않는다. 그러면 세 지적이 **성립할 수 없는 형태**가 된다.
(이 저장소의 계보: 같은 자리가 반복해 뚫리면 술어가 아니라 계열을 바꾼다.)

r2 봉합 시험 = 항목별 만기 왕복(옛 것만 나가고 새 것은 상속 · from_surface 보존까지) + 스윕 2건
(정지 상태 발행 + 만기 전 무접촉 음성 대조) + 배선 핀. 뮤턴트 5건(M-D7-15~19) 전부 KILLED.
