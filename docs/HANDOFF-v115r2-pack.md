...
### ★종단 실측 — 좌석 claude 가 실제로 스킬 목록에 보였다 (2026-09-23 06:17 · 사고성 관측)
`preflight.c29_harness_engineering()` 을 `--fix` 로 돌리자 **`~/.cys/claude/skills/dept-by-chat`**
심링크가 생겼고, **그 순간 이 워커 좌석의 Claude Code 가 `dept-by-chat` 을 사용 가능 스킬로
표시**했다(하네스가 스킬 목록 갱신을 통보). 브리프가 요구한 「좌석이 스킬 목록에서 보는가」의
종단 증거가 이것이다 — 등록만 되면 좌석이 즉시 본다.

⚠**정직 고지 — 이 관측은 의도한 것이 아니었다.** 격리 프로브를 돌리며 `CYS_ACCOUNT_DIR` 만 끊고
**내 셸의 `CLAUDE_CONFIG_DIR`(= 라이브 `~/.cys/claude`) 을 안 끊어서**, 프로파일 발견이 라이브
프로필을 집었고 `--fix` 가 거기에 심링크 2개(dept-by-chat·harness-engineering)를 실제로 썼다.
링크가 프로브 팩(`.probe/pack/...`)을 가리키고 있었으므로 **둘 다 즉시 제거**했고
`~/.cys/claude/skills` 는 프로브 전 상태(`synced/` 만)로 복구했다 — 라이브 잔재 0.
같은 오염(상속된 env)을 이 라운드에서 **두 번** 냈다(D4 대조군 1차 · 여기). 격리 프로브는
`env -u` 로 **끊을 env 를 목록으로** 관리해야 한다. ★그리고 이 사고는 preflight 의 임시팩 가드
(`_path_under_tempdir(pack_dir())` → 실 config 등록 절대 금지)가 **팩만 보고 프로필은 안 본다**는
것을 드러낸다 — 팩이 임시가 아니면(내 프로브처럼 저장소 안) 가드가 열린다. 가드 확장 여부는
master 판단 대상(이번 판 범위 밖 · 이월 7).
...
7. **preflight 임시팩 가드의 범위**(위 §2 종단 실측 각주): `_discover_isolation_block()` 은
   **pack_dir 이 임시 경로인가**만 본다. 팩이 임시가 아니면 `--fix` 가 라이브 프로필에 쓴다 —
   프로브·시험 하네스가 라이브를 건드릴 수 있는 창이다. 「대상 프로필이 실 프로필인가」 축을
   가드에 더할지는 master 판단(안전 이득 vs 정상 설치 경로 차단 위험).
