...
2. **막힌 번호**(판정은 **메모리 holders 상태만** 본다 · surfaces 맵 조회 금지 — 할당~맵 등록 사이, 닫기 사이의 창에서도 같은 답을 내기 위해 · Fable 5R): 번호 `n` 의 마지막 주인이 ⑴ `Live`(산 좌석 · 끝났지만(exited) 아직 회수 안 된 좌석 · 할당은 됐지만 아직 맵에 안 들어간 좌석 포함) 또는 ⑵ 닫힌 지 W(24시간) 가 **안 됐다**(`now − closed_at < W` · 엄격한 미만 — 정확히 24시간이 지난 순간부터 다시 쓸 수 있다).
3. `c` 가 막혀 있지 않으면 `c` 를 준다. 막혀 있으면 `c+1, c+2, …, 999, 1, 2, …, c−1` 순서로 **처음 비어 있는 번호**를 준다.
4. **I1 보호 규칙**: `id ≤ 999` 인데 `c(= id)` 가 막혀 있으면 탐색하지 않고 **번호 없음(「—」)** 으로 만든다 + 경보 1줄. (정상이라면 일어날 수 없다 — 증명 §2-3. 일어났다면 표가 **바깥에서 손상**됐다는 뜻이므로(손으로 고친 행 · 옛 백업과 섞인 파일 — 이 설계 자신의 쓰기 실패로는 도달하지 않는다 · §2-3) 조용히 다른 번호를 주지 않고 드러낸다.)
...
- ⑤**DB 쓰기가 실패하면**(디스크 가득 · 잠김 시간 초과): 좌석은 **만든다**(4군 ③) · 메모리 할당기로 번호를 정한다 · 경보 1줄(`surface.numbers_alarm{kind:"write_io"|"pk_conflict"}`). 그 좌석은 I0 보호가 빠진 상태(재기동 뒤 내부 번호 재사용 가능)라는 것을 경보 문구에 적는다. 그 뒤 재기동해 내부 번호가 다시 나와도 S 는 성립한다 — 못 쓴 행은 표에 없으므로 holders 에도 없고, 실행 안의 I1 은 구성으로 성립한다(§2-3 · 이 경우 I1 보호·번호 정지는 **발동하지 않는다** · 드러내는 신호는 `pk_conflict`/`write_io` 경보). **`pk_conflict` 가 난 좌석은 이후 그 행에 대한 UPDATE(`spawn_failed`·`close`)를 하지 않는다** — 이미 있던 남의 행을 덮어써 그 번호의 막힘을 일찍 풀지 않기 위해(Fable 5R).
...
- ⚠**DB 를 못 읽으면 시드가 조용히 0 이 된다**(Fable 1R LOW-4 · recall.rs:387-408 `.ok()…unwrap_or(0)` 【관측】) → 내부 번호가 1 부터 다시 나와 I0 이 깨진다. 처방: 「DB 파일은 있는데 열기·질의가 실패」 를 「DB 파일 없음(새 설치 = 0 이 맞음)」 과 갈라 **경보 1줄**(`surface.numbers_alarm{kind:"seed_failed"}`)을 낸다. 그 실행에서도 S 는 성립한다 — 표를 못 읽었으니 holders 가 비어 있고 실행 안의 I1 은 구성으로 성립한다(I1 보호·번호 정지는 발동하지 않음 · §2-3). INSERT 가 PK 충돌로 실패하면 §3-2 ⑤ `pk_conflict` 경보가 난다. (시드를 대신 채울 다른 원천 — 예: 별도 카운터 파일 — 은 범위 밖 · 경보로 드러내는 데서 멈춘다.)
...
| T13 | DB 쓰기 실패 생존(agy 1R 지적 3) | 통합(격리 state_dir) | 대응표 쓰기를 실패시킨 채(**전체 스키마를 먼저 만든 뒤** 읽기 전용으로 바꾼 DB 파일 — 스키마 없이 읽기 전용이면 ⑴이 실패해 `seed_failed` 가 함께 뜬다 · Fable 4R · 또는 쓰기 실패 주입 지점) 좌석 생성 → 좌석은 **정상 반환** · 좌석 쓰기마다 `surface.numbers_alarm{kind:"write_io"}`(`surface_id` 있음 — 부팅 때 고아 UPDATE 실패로 나는 `surface_id` 없는 1건과 구별해 센다 · Fable 5R) · 보이는 번호는 메모리 규칙대로 · 같은 실행 안에서 I2 유지 | M24 쓰기 실패를 생성 실패로 전파 → 적색(4군 ③) |
...
| R-3 | 대응표 쓰기 실패 중 재기동 → 내부 번호 재사용 | 좌석은 만듦 · 경보 · S 는 그대로 성립(못 쓴 행은 holders 에 없음 · 실행 안 I1 은 구성으로 성립) · 신호 = `pk_conflict` 경보 · I1 보호·번호 정지는 여기서 발동하지 않음(바깥 손상 전용 · §2-3) | 잔여: 그 좌석만 X-10 노출 · 경보로 드러남 |
...
| R-22 | transcripts.db 를 못 읽어 시드 0 → 내부 번호 1 부터 재발급 | 경보 `surface.numbers_alarm{kind:"seed_failed"}` · holders 비움 → S 성립 · `pk_conflict` 경보(§6 · §3-2 ⑤) · I1 보호·번호 정지는 발동 안 함 | 잔여(드러냄까지 · 자동 복구 없음) |
...
### 14-2. 2회차(완료 전 · 5R 반영 뒤)

| 단계 | 적용 | 이유·결과 1줄 |
|---|---|---|
| 1 철학·로컬·구독·품질 | 적용 | 검증을 속도에 맞춰 줄이지 않았다 — 두 검증자 5라운드 · 셸 주장은 추론 대신 실측 7+3건으로 판정 |
| 2 구체 설계안 | 적용 | 부팅 순서(⓪~⑷)·락 범위(락 안 DB 쓰기 0)·경보 이름 하나가 라운드마다 구체화됐다 |
| 3 의도·영향 범위 | 적용 | 영향 범위가 1R 뒤 넓어졌다: `cys list` 소비자 9+1곳 · 사용자본 2개(panetitle · reconstruct_state) · 기계 출력 줄 — 모두 §4-1·§10 에 반영 |
| 4 설계 결함 재조사 | 적용 | 실제 결함 3건을 검증이 잡았다 — 업그레이드 첫 기동 시드 0(2R) · 새 설치 거짓 경보(3R) · 읽기 전용 디스크에서 읽기까지 버림(4R). 셋 다 「부팅 순서」 한 곳에 몰려 있었다 → ② 구현 리뷰의 조준점 |
| 5 할루시네이션 위험 | 적용 | **내가 1R 에서 옳은 결론(셸 절단 = clap 오류)을 검증자 주장에 밀려 「정정」했다가 2R 에 실측으로 되돌렸다** — 검증자 주장도 도구 출력으로 확인한 뒤에만 반영한다는 교훈 · 이후 모든 반영에 코드 행 확인을 붙였다 |
| 6 적대 A/B/C | 적용 | 운영 실패(부팅·디스크) = Fable · 단순성 = agy(I1 보호 삭제 권고 — 부분 수용) · 유지보수(문서 정합) = Fable 5R |
| 7 언어 원칙 | 부분 | 저장소 관례(한국어 설계 문서)를 따름 · 필드·이벤트·RPC 이름은 영어(`display_no` · `surface.numbers_alarm` · `surface.resolve_display`) |
...
### 15-5. 5R (9042de8c 대상 · 4R 개정분) — 수렴 판정

- 원문: `surface-display-number.reviews/agy-5R-2026-09-24.md` · `surface-display-number.reviews/fable-adversarial-5R-2026-09-24.md`.
- **agy = ACCEPT · 발견 없음**(dry — 락 밖 INSERT·되돌림 경합 없음 · 부팅 분리 유효 · I1 보호 유지 근거 타당 · 다른 산 창 경로 없음).
- **Fable = REVISE**(MED 1 · LOW 3) — 단 **메커니즘 결함 0**: 「세 메커니즘(락 밖 INSERT · 부팅 순서 · I1 보호 유지)은 성립」 · 할당 직후 내부 번호가 처음 나가는 자리(state.rs:3503 openpty 뒤 · env :3627 · spawn :3654 · 맵 :3823 · 이벤트 :3865)를 대조해 I0 성립 확인. 발견은 전부 **문서 정합성·정밀도**였다:

| 발견 | 처리 | 반영 |
|---|---|---|
| MED 4R 에서 「I1 보호는 설계 자신의 경로로 도달 불가」 로 고쳤는데 다섯 곳이 여전히 「그 경우 I1 보호가 S 를 지킨다」 고 적음 | 수용 — 다섯 곳을 「S 는 holders 가 비거나 실행 안 I1 로 성립 · 신호는 경보 · I1 보호는 바깥 손상 전용」 으로 | §2-1 ④ · §3-2 ⑤ · §6 · R-3 · R-22 |
| LOW 막힘 판정은 holders 만 봐야 함(맵 조회 금지) | 수용 | §2-1 ② |
| LOW 시험의 경보 개수는 `surface_id` 유무로 갈라 세야 함 | 수용 | T2 ⑹ · T13 |
