...
M = [
 ...
 ("M22 14:5x 표지 무시(창 크기 바뀌어도 안 잼)", "src/formation.ts", "(tree as { leftAuto?: boolean }).leftAuto === true ||", "false ||"),
 ("M23 14:5x 옛 기본 1/2·1/3 이동 없음", "src/formation.ts", "|| OLD_DEFAULT_SHARES.some((v) => Math.abs(cs - v) <= RULE_TOL);", "|| false;"),
 ...
 ("M30 Fable R2 좌열 = 순서 첫 좌석(옛)", "src/formation.ts", "const left = leftSids([...order.filter((s) => had.has(s)), ...order.filter((s) => !had.has(s))], roles);", "const left = leftSids(order, roles);"),
