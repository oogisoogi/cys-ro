...
M=[
 ...
 ('M8c','src/updateplan.ts','toastMsg: `팩 ${i.packVersion} — 상단 「업데이트」(재시작 없음)`,','toastMsg: `팩 ${i.packVersion} — 상단 Update 버튼(재시작 없음)`,','src/topbarlabels.test.ts'),
 ...
 ('M12b','src/main.ts','  if (isMasterSeatSignal(name, payload)) maybeShowRestoreBrief();\n','','src/restorebrief.test.ts'),
 ...
 ('M13c','src/panetitle.ts','return before && before.startsWith(`${sid}${SEP}`) ? before : "";','return "";','src/panetitle.test.ts'),
 ...
 ('M14f','src/main.ts','        if (r && !Number.isFinite(usedPctOf(r.used_pct))) r = undefined; // (D4 #18) 미관측 = 창 없음(「—」) · 0% 게이지 아님\n','','src/wsusage-null.test.ts'),
 ('M14g','src/wsusage.ts','  return v == null || v === "" ? NaN : Number(v);','  return v === "" ? NaN : Number(v);','src/wsusage-null.test.ts'),
 ('M15a','src/alertcopy.ts','의 AI가 종료됐습니다. 창과 작업 폴더는 그대로 남아 있습니다. ${CHECK}','의 AI가 종료됐습니다. ${CHECK}','src/alertcopy.test.ts'),
 ('M15b','src/alertcopy.ts','한 번 정리할 때가 됐고, 정리는 총괄이 맡습니다.`','한 번 정리할 때가 됐고, 정리는 총괄이 맡습니다. ${p.action ?? ""}`','src/alertcopy.test.ts'),
 ('M15c','src/main.ts','    const c = approvalRequestCopy(no, payload);\n    toast("approval", c.title, c.body);','    const c = approvalRequestCopy(no, payload);\n    toast("approval", c.title, `${payload.role ?? ""} surface:${sid} — ${c.body}`);','src/alertcopy.test.ts'),
 ...
 ('M15f','src/alertcopy.ts','title: "❌ AI가 멈췄습니다"','title: "AI가 멈췄습니다"','src/alertcopy.test.ts'),
