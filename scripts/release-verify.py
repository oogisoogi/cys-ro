...
★`SHA256SUMS.txt` 를 정본으로 삼을 때의 구멍과 그 메움 (2026-08-18 적대적 리뷰로 교정)
  ...
  그대로 두면 검증이 fail-open 이다. 구멍을 세 겹으로 메운다:
    (1) `REQUIRED_ASSETS` — SUMS 와 무관한 독립 하한선(플랫폼 무관 + 윈도우 레인 6종)
        ...
        ★맥 레인 2종은 2026-09-09 에 `MAC_ASSETS` 로 갈라져 `decide_mac_lane` 이
        **전부-또는-전무**로 판정한다(아래 §맥 레인 참조). 그 2종은 2026-09-20 에
        DMG → **배포 zip** 으로 조준이 옮겨졌다(`MAC_ASSETS` 주석 · TICKET=v110-mac-lane).
  ...
  (1)+(2) 로 맥 포함 묶음 13종이 **전부** 덮인다: 필수 6 + 맥 레인 6(배포 zip 2 + 업데이터 4) +
  윈도우 exe 서명 1 = 13 —
...
★맥 레인 — 전부-또는-전무 (2026-09-09 · TICKET=cys-release-first-publish)
  ...
  성립해야 한다. 그렇다고 맥 다운로드 자산 요구를 그냥 지우면 이 파일이 원래 막던 것 —
  「맥 사용자에게 반쪽만 도달하는 묶음」 — 이 다시 열린다. 그래서 요구를 **지우지 않고 옮겼다**:
    · `decide_mac_lane()` 이 디렉터리 실물로 맥 레인 6종(배포 zip 2 + 업데이터 tar 2 + 서명 2)의
      존재를 세어 **전부(6) 또는 전무(0)** 만 인정한다. 1~5종 = 즉사.
      ★2026-09-20(TICKET=v110-mac-lane): 첫 2종이 DMG 에서 **배포 zip** 으로 바뀌었다. 우리 포크는
      DMG 를 만든 적이 없어 종전 상수는 **영원히 없는 자산**을 요구했고, 그 때문에 맥이 실재하는
      묶음이 「반쪽」으로 판정돼 발행이 막혔다(v1.1.0 후처리 실측 오류: 「배포 자산 누락:
      cysr_1.1.0_aarch64.dmg, cysr_1.1.0_x64.dmg」). 계약의 **개수와 전부-또는-전무는 불변**이고
      조준만 실재 자산으로 옮겼다.
    · 그 판정(`mac_included`)이 그대로 `check_latest_json` ② 의 **기대 플랫폼 키 집합**이 된다
      (윈도우 단독 2키 · 맥 포함 6키). 디렉터리와 업데이터 표면이 갈리면 여기서 죽는다 —
      배포 zip 만 있고 darwin 행이 없는 묶음도, darwin 행만 있고 배포 zip 이 없는 묶음도 통과 못 한다.
...
#   「배포 zip 은 있는데 darwin 행이 없다」도, 「darwin 행은 있는데 배포 zip 이 없다」도 여전히 죽는다.
...
# ★맥 레인의 다운로드 버튼 자산 = **설치 사이트가 받는 zip**.
#   (2026-09-20 · TICKET=v110-mac-lane · master 기술 판정) 종전에는 이 칸이 DMG 2종이었다.
#   그것은 **벤더 시절 가정**이다 — 원작자 판은 DMG 를 만들고 사람이 손으로 끌어 넣었다.
#   우리 포크는 DMG 를 만든 적이 없다: 유료 Apple 서명이 없어 CI 맥 레그가 비발행이고,
#   로컬 자체서명(cys-local) 빌드 산출은 zip 뿐이다. 손 설치 경로도 DMG 가 아니라
#   **설치 사이트가 이 zip 을 받아 푸는 것**이다(release-postprocess.py `MAC_DIST_ZIPS` 주석).
#   ⇒ 없는 자산을 요구하던 칸을 **실재하는 자산으로 조준을 옮겼다**(완화가 아니라 이동 —
#     레인 전집합의 개수 6은 그대로이고 `decide_mac_lane` 의 전부-또는-전무도 그대로다).
#   ⚠DMG 는 「거부」가 아니라 「요구하지 않음」이다. 섞여 들어오면 `ASSET_SHAPES` 의 `.dmg`
#     규칙이 그대로 껍데기를 검사한다(무단 자산 유입 차단 계약 유지).
#   업데이터 4종(tar.gz 2 + .sig 2)은 `MAC_PLATFORMS` 가 강제하며,
#   `mac_lane_files()` 가 이 둘을 합쳐 "맥 레인 전집합 6종"을 만든다.
MAC_ASSETS = (
    "cysr-macos-arm64-v{v}.zip",   # macOS Apple Silicon 다운로드 버튼(설치 사이트가 받는 zip)
    "cysr-macos-x64-v{v}.zip",     # macOS Intel 다운로드 버튼(설치 사이트가 받는 zip)
)
...
def mac_lane_files(version):
    ...
    배포 zip 2종(`MAC_ASSETS`) + 업데이터 tar.gz 2종 + 그 서명 2종(`MAC_PLATFORMS` 의 값에서 유도).
    (2026-09-20 전환 전에는 첫 2종이 DMG 였다 — `MAC_ASSETS` 주석 참조. 개수·전부-또는-전무 계약 불변.)
