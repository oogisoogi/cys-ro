        ...
        "surface.create" => {
            ...
            match daemon.create_surface_with_env(
                ...
                Ok(s) => {
                    ...
                    // ★v115r3-d7(D7⑴): 좌석 캐시를 생성 직후 1회 채운다 — 응답 전에 두어야 호출자의
                    //   다음 status 가 "unknown" 을 보지 않는다(부서 allocate → 편성 즉시 판정의 경합).
...
mod tests {
    ...
    fn v115_seat_takeover_notice_is_screen_output_not_shell_input() {
        ...
        // ★(v116-integ · master#24673176) GUI 문안의 원본이 main.ts → ui/src/alertcopy.ts(roleTakeoverCopy)로 옮겨졌다
