...
//   · 마스터 자리 1곳에만 띄운다. 「이어서 진행할까요?」 버튼은 사용자가 누를 때만 마스터에 한 줄을 보낸다
//     (자동 재개 주입 0 — 복원 정책 「재개 지시는 넣지 않으며, 일은 사용자의 한마디 뒤에 잇는다」).
...
export interface BriefCard {
  ...
  closeLabel: string;
  /** [이어서]를 눌렀을 때 마스터에 보내는 한 줄(사용자가 누를 때만 나간다). */
  continueText: string;
}
...
export function buildBriefCard(input: {
  ...
  return {
    title: "다시 켜졌습니다 — 멈춘 자리부터 보여 드립니다",
    lines,
    foot,
    continueLabel: "이어서 진행할까요?",
    closeLabel: "닫기",
    continueText: "복원 안내에서 [이어서 진행]을 눌렀습니다. 남은 일을 먼저 짧게 알려 주고, 이어서 진행해 주세요.",
  };
}
